...
test('AC4-b: 行削除で draft 退避（編集対象行を削除 → draft を Conflict Queue 相当へ退避・無効 RowId へ Commit しない）', async ({
  browser,
}) => {
  const alice = await client(browser, 'Alice');
  const bob = await client(browser, 'Bob');
  ...
  // A へ伝播 → 編集対象行が tombstone 化 → draft を退避（黙って破棄しない）・editingTarget 解除。
  await expect
    .poll(async () => (await snapshot(alice)).divertedCount, { message: 'draft 退避（AC4）' })
    .toBe(1);
  await expect
    .poll(async () => rowIndexOf(alice, editRowId!), { message: '編集行が消失（tombstone）' })
    .toBe(-1);
  const afterDelete = await snapshot(alice);
  expect(afterDelete.editingTarget, '編集対象は解除').toBeNull();

  // 無効 RowId へ Commit しない: 退避後は textarea が隠れ focus も外れる。stray Enter（body 宛て）でも submit されない。
  await alice.keyboard.press('Enter');
  await alice.waitForTimeout(300);
  const afterEnter = await snapshot(alice);
  expect(afterEnter.divertedCount, '退避 draft は保持（破棄しない）').toBe(1);
  expect(afterEnter.pendingCount, '無効 RowId への submit なし').toBe(0);
