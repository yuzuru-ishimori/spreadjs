...
describe('selection-controller: rebaseRows（K3 行構造変更後の再ベース・DD-021-3）', () => {
  ...
  it('ドラッグ中のライブ状態も再ベースする（ドラッグ中のリモート Insert で index シフト・Fable P2）', () => {
    const c = createSelectionController();
    c.beginDrag({ row: 2, col: 1 });
    c.updateDrag({ row: 4, col: 3 });
    const changed = c.rebaseRows((r) => r + 2); // 上に 2 行挿入相当
    expect(changed).toBe(true);
    // pointerup（endDrag）で確定するレンジがシフト後の行を指す。
    expect(c.endDrag()).toEqual({ rowStart: 4, rowEnd: 7, colStart: 1, colEnd: 4 });
  });

  it('ドラッグ中に anchor 行が削除されたらドラッグを取消す（ずれた行帯の確定を防ぐ）', () => {
    const c = createSelectionController();
    c.beginDrag({ row: 2, col: 1 });
    c.updateDrag({ row: 4, col: 3 });
    const changed = c.rebaseRows((r) => (r === 2 ? null : r)); // anchor 行が消えた
    expect(changed).toBe(true);
    expect(c.isDragging()).toBe(false);
    expect(c.endDrag()).toBeNull(); // 確定しない
  });

  it('ドラッグ index 不変なら false（下に挿入）・ドラッグ継続', () => {
    const c = createSelectionController();
    c.beginDrag({ row: 2, col: 1 });
    c.updateDrag({ row: 4, col: 3 });
    expect(c.rebaseRows((r) => r)).toBe(false);
