...
//
// これは試験ハーネスゆえ Room（server）に依存してよい（client-session 本体 session.ts は非依存を維持）。
// フォールト注入は Math.random ではなくシード付き mulberry32（同一シード→同一実行＝S-M2 の再現性）。

import type { ClientMessage, ServerMessage } from '@nanairo-sheet/core';
import { Room } from '@nanairo-sheet/server';
import type { Outbound, OutboundTarget } from '@nanairo-sheet/server';
