// @nanairo-sheet/sheet-collaboration の公開エントリ。共同編集クライアント（ClientSession＝楽観適用・§7.7
...
//
// この本体エントリはトランスポート非依存・外部ランタイム依存ゼロ（sheet-core / sheet-types の型・関数のみ）。
// 試験専用の in-process ハーネス（InProcessHub＝Room 依存）とテストビルダーは本体を汚さないよう
// サブパス（`@nanairo-sheet/sheet-collaboration/inprocess-transport`・`/test-support`）でのみ公開する。
export * from './session';
export * from './deps';
