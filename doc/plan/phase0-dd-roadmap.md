...
- **DD-002 はトレース先行方針**（2026-07-11 ユーザー指摘）: 状態機械を机上で先に固定せず、最小の常駐textarea＋生イベントrecorderを先に作り、実IME（MS IME／Google日本語入力 × Chrome／Edge）のトレースを短時間採取してから scenarios.md と状態機械を確定する。
- **プロセス方針**（2026-07-11 ユーザー指摘）: DD合意済みスコープ内の内部詳細は各Phaseで停止せず、DA・テスト・Codexレビュー通過で継続する。停止は仕様・受け入れ基準・ユーザー体験の変更時と、明示的なユーザー実機ゲートのみ。
