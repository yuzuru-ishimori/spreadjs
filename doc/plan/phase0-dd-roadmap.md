...
| ② | - | PoC-A 日本語IME | 20行×10列Canvas＋常駐textarea＋composition event recorder＋リモート更新シミュレーター＋スクロール追従（§18.1・§11） | イベントトレース、合格条件判定（**実IME手動試験＝ユーザー実機作業を含む**） |
| ③ | - | PoC-B Canvas・仮想スクロール | 50,000行×200列、可変行高・列幅、固定行列、Presence overlay 20人、高DPI（§18.2・§12・§13） | fps・メモリ計測、scroll anchor検証 |
| ④ | - | PoC-C 共同編集・Operation | `sheet-core`最小（固定ID/Axis/CellStore）＋in-memoryシーケンサー＋WS同期＋楽観適用rollback/replay＋再接続・冪等性（§18.3・§7・§8） | ランダムOperation収束試験（全クライアントhash一致）、ADR-005/008ドラフト |
...
- ③④⑤は①完了後なら入れ替え・並行可（計画書§19「並行開発」）。既定順は ②→③→④→⑤。
