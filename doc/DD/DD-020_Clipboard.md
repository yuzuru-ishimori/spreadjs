...
| 作成日 | 更新日 | ステータス | 補足 |
|--------|--------|-----------|------|
| 2026-07-16 | 2026-07-16 | 検討中 | アンブレラ（子: DD-020-1 範囲選択 / DD-020-2 clipboard / DD-020-3 Undo/Redo） |
...
- [ ] DD-020-1 の全Phase完了確認（📐選択所有者設計〔案X〕→E2E Red→実装→Codex high→親AC1/2 充足・引き継ぎ物=選択読み取り・range-ops・上限語彙）
- [ ] 🔬 **機械検証**: `npm run test && npm run typecheck && npm run lint` → 全 green（CI も push で検証）
...
- [ ] DD-020-2 の全Phase完了確認（TDD Red→Green→E2E→Codex high→親AC3〜7/9(paste)/10 充足・引き継ぎ物=確定単位 chokepoint）
- [ ] 🔬 **機械検証**: `npm run test && npx playwright test`（apps/playground） → 全 green
...
- [ ] DD-020-3 の全Phase完了確認（TDD→Codex high→親AC8/9(undo) 充足・既知制約「クリア/貼り付けの Undo なし」解消）
