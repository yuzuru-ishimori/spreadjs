...
| DD | 件名 | ステータス | 補足 |
|----|------|-----------|------|
| DD-012-4 | 列幅行高リサイズ | 検討中 | 仕様確定済（ユーザー 2026-07-15: ローカル＋利用側保存・F5反映は初期値注入で実現）。実装は dd-implementer |
| DD-012-5 | オーバーフロー表示折り返し自動行高 | 検討中 | 仕様確定済（ユーザー 2026-07-15: 列単位wrap・自動行高=Excel風）。**DD-012-4 完了後に着手**（行高 override 機構に依存） |
