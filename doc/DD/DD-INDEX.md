...
| DD-006 | PoC-Dデータ表現・簡易数式 | 進行中 | DD-005完了→着手。**Phase 1（CellStore 4実装）＋Phase 2（数式parser・固定IDバインド）実装済**（`apps/pocd-bench`＋`packages/sheet-formula`・test 499件green・回帰0・typecheck:core green）。外部レビュー2回反映済み。Phase 3以降（依存グラフ/評価器/replay/レポート/ADR/Codex）未着手 |
