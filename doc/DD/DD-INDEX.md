...
| DD | 件名 | ステータス | 補足 |
|----|------|-----------|------|
| DD-021 | 行操作 | 進行中 | 要確認①〜⑦確定（既定案どおり）・子DD 3本（DD-021-1〜3）起票済み。実装は DD-021-1 から |
| DD-021-1 | 行操作Command公開API | 進行中 | 親=DD-021（3分割の第1子）。実装・全検証 green。**Codex レビューのみ次セッション持ち越し**（本環境 --check exit 1）。完了/アーカイブは親 DD-021 完了時 |
| DD-021-2 | 行操作収束・競合 | 進行中 | 親=DD-021（3分割の第2子）。実装・全検証 green。**Codex レビューのみ次セッション持ち越し**（本環境 --check exit 1）。完了/アーカイブは親 DD-021 完了時 |
| DD-021-3 | 選択再ベース・性能 | 確認待ち | 親=DD-021（3分割の第3子・実装順3番目）。K3 回収・Undo 生存整合・P2-1 Θ(N²) 是正。実装完了・全検証 green・**Codex レビュー未実施（不可・次セッション要）**・未コミット |
