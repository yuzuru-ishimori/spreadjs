...
> ⚠️ **Phase 6 は「確認作業」ではなく「正式な設計検証」**（2026-07-12 レビューアー指摘）。
> 本DDは当初のトレース先行方針（実IMEトレースを先に採取→状態機械を確定）を途中で変更し、
> **状態機械を合成リファレンス先行で実装**して実IME検証を Phase 6 へ送った。したがって Phase 6 で
