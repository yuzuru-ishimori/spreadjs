...
| 作成日 | 更新日 | ステータス | 補足 |
|--------|--------|-----------|------|
| 2026-07-15 | 2026-07-15 | 完了 | compositionend の base+data 近似上書きを sawCompositionInput で guard（キャレット位置保持）。S-C6/S-C7 追加・746 test＋invariants＋E2E 8本 green・Codex high findings 0・**実IME実機確認 OK（ユーザー 2026-07-15）＝AC1〜4 全充足** |
...
- [x] 実機確認（ユーザー・AC4）: 報告の再現手順で「いいい柿食えば」を確認（2026-07-15 OK）
...
- **実IME実機確認 OK（ユーザー 2026-07-15）→ 完了・アーカイブ**。仕様書同期: `doc/spec/` 未設置のためスキップ。知見昇格: 「compositionend の確定値は変換中 input の実値を優先（キャレット末尾前提の近似で上書きしない）」は S-C6/S-C7 テスト＋状態機械コメントに固定済み＝別文書への昇格不要と判断
