...
DAレビューは各 Phase の 😈 タスクで実施（Phase 0 事前調査／Phase 1 契約／Phase 2 実装／Phase 3 E2E／Phase 4 DX）。
Codex(high) レビューが Phase 3 で独立視点の敵対的検証を担い、下表に主要発見を統合する。

**DA観点:** Facade で最も壊れやすいのは「React のライフサイクル/並行性」と「grid 内部状態の React への漏れ」。

| # | 発見した問題/改善点 | 重要度 | 再現手順（高/中は必須） | DA観点 | 対応 |
|---|-------------------|--------|----------------------|--------|------|
| 1 | render 中の callback ref 更新は Concurrent React（startTransition/Suspense）で未 commit render の callback が現行 grid へ漏れ、A の cell-commit が B の保存先へ届く | 高 | 親を startTransition で B へ遷移→保留中に A の grid で確定→B の onCellCommit が呼ばれる | ライフサイクル/並行性（Codex P1a） | ✅修正済（`useLayoutEffect` で commit 後に ref 反映） |
| 2 | 初期値系の変更検知で `initialData` を毎 render `JSON.stringify` → 数万行で render が同期停止 | 高 | 数万行 initialData を渡す consumer で親が頻繁に再 render → 体感カク付き | 性能（Codex P1b） | ✅修正済（参照比較 Object.is へ変更） |
| 3 | identity 不安定 props（毎 render 新規の `columnOrder`/`wrapColumns` 配列）が意図せぬ全 remount を誘発 | 中 | `columnOrder={[...]}` を毎 render リテラルで渡す→毎回 destroy→mount | ライフサイクル | ✅修正済（mountKey の値比較で吸収・unit で固定） |
| 4 | onDiagnostic の後差し替えが grid に伝わらない／mount 前 pending で lastConnState が不正値 | 中 | onDiagnostic を A→B 変更後の boot 診断が A のまま／初回 connection 前の pending が 'stopped' 通知 | 契約一致（Codex P2a/P2b） | ✅修正済（安定ラッパー＋mount 時 connectionState 初期化） |
| 5 | `.tsx` ソース配布は consumer（DD-026 Vite）で JSX 変換設定に依存し手戻りうる | 中 | consumer の esbuild/jsx 設定次第で配布物が変換不能 | DX/互換性 | ✅回避（配布本体は `.ts`＋createElement・契約 §7） |
| 6 | grid 内部状態（文書/レイアウト/接続状態）を React state/props へ複製してしまう誘惑 | 高 | data prop 宣言的再注入案（要確認②案b）を採ると文書を React state で保持する圧 | 憲章 §11.2 逆行 | ✅回避（ref.setData 一択・state 非保持を unit/E2E で固定） |
| 7 | synthetic composition E2E と実 IME 変換の乖離（jsdom unit は grid を mock し実描画/実 IME を代弁できない） | 中 | jsdom/synthetic だけでは実 IME 変換確定の回帰を捕捉できない | 検証の妥当性 | ✅補完（実ブラウザー E2E＋Manual Gate 実機 IME で二段担保・OK） |
