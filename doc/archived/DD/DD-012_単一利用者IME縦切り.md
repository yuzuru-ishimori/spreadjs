...
| 作成日 | 更新日 | ステータス | 補足 |
|--------|--------|-----------|------|
| 2026-07-13 | 2026-07-14 | 完了 | 案Y 2分割・両子DD完了アーカイブ済（DD-012-1＝CG-1解除／DD-012-2＝CG-6指標pass）。milestone残（ime/selection/render物理抽出・baseline縮退・CG-1統合後スモーク・CG-6精密メモリ）は**DD-016で確定＝完了**（DD-016-1 抽出・baseline縮退／DD-016-2 CG-1統合後スモークPASS・CG-6精密メモリPASS＋redraw境界化）。**AC2/AC4 充足＝本アンブレラ クローズ（2026-07-14）** |
...
- **DD-012-1 進捗**: Phase 1〜3 完了（型変換 標準セット・CellScalar date・ローカルOp・hash 決定性・IME 不変条件 6 項目・ADR-012 Accepted・Codex xhigh findings 5件全対応）。証跡=`doc/DD/DD-012-1/`。

### 2026-07-13（DD-012-1 完了・CG-1 解除）
- **DD-012-1 完了**（Phase 1〜4）: **CG-1 実機ゲート PASS**（Win Chrome/Edge・Microsoft IME・20セッション・先頭欠落0＋順序B・`judge-ime-trace.mjs` verdict PASS）。順序A は Chromium 150 で構造的に不発→自動テスト担保（ユーザー承認 2026-07-13 の再定義）。CG台帳 CG-1「解除済」。milestone AC1（DD-012-1 完了・CG-1 解除証拠）充足。**DD-016 の統合後 Tier 1 実機スモークは残**。次=DD-012-2（性能縦切り・CG-6）。

### 2026-07-13（DD-012-2 完了・CG-6 指標計測）
- **DD-012-2 完了**: 性能予算の常設化（`scripts/cg-perf/perf-budget.json`＋判定器＋`tests/invariants/perf`）・計測ハーネス（pocb 流用）・**Phase 2/3 指標計測**（Playwright MCP Chrome150）・evidence full・Codex high（8件全対応）を実施。指標=scroll p95 **16.8ms pass**・選択 17.0ms pass・メモリ peak **24.2MB ≪ 300MB pass**・リークなし／再描画 0.845ms は **over-budget（render 無変更ゆえ回帰不能の計測環境アーティファクト）**。
- **render 抽出は DD-016 委譲（要判断・ユーザー承認）**: 今抽出すると apps(integration)→render R1 が正味肥大＋render→chunk-store/presence-sim/selection の R4 で、grid Facade＋selection package 化（DD-016）なしにクリーン抽出不可（DD-012-1 と一貫）。→ milestone AC2/AC4 の「render 抽出・baseline 縮退」は DD-016 依存。
- **CG-6**: 指標的 pass（24MB≪300MB）。CG台帳「証拠待ち（指標）」。**精密メモリ（`--enable-precise-memory-info`）＋clean redraw の定義的解除は DD-016 統合後実機スモークで確定**（CG-1 の DD-016 残スモークと同型）。
- **milestone 依存反映**: 本アンブレラの完了（AC2/AC4）は **DD-016（render 抽出・Facade 配線・統合後実機スモークで CG-6 精密／CG-1 最終確認）** に依存。DD-012-1（CG-1 指標＋実機）・DD-012-2（性能予算常設化＋CG-6 指標）は完了。

### 2026-07-14（DD-016 完了によりクローズ）
- **DD-016 完了連絡（DD-016-2 より）**: 残 milestone が DD-016 で確定。**AC2（lifecycle・再mount leak）**＝DD-016-1（Facade lifecycle 契約）＋DD-016-2 Phase 3（consumer-app 再mount×5 leak なし）で充足。**AC4（ime/selection/render 物理抽出・baseline 縮退）**＝DD-016-1（baseline 41→10・new=0）で充足。**CG-1 統合後スモーク PASS**（DD-016-2 Phase 4・Chrome6＋Edge3＝9 sessions・先頭欠落0・順序B）・**CG-6 精密メモリ PASS**（DD-016-2 Phase 4・peak 65.3MB≪300MB・純減リークなし・redraw 境界化）＝cg-ledger CG-1/CG-6 解除済。
- → **本アンブレラ（DD-012）AC1〜4 全充足＝クローズ**。証拠は各子DD・DD-016 系 evidence（`doc/DD/DD-016-2/`・`doc/archived/DD/DD-016-1/`・`doc/archived/DD/DD-012-{1,2}/`）。
- **DA批判レビュー**: 本DDは管理アンブレラ（実作業・ゲート・DAは子DDで実施）ゆえ本体の DA表は雛形のまま＝**DA は子DD（DD-012-1/012-2・DD-016-1/016-2）で記録済み**（アーカイブ時の雛形残置は仕様どおり・不要）。
