...
interface RunOptions {
  ...
  minLiveRows: number;
  hotCells: number;
  // DD-021-2: 行操作比率の強化（既定は従来値＝既存 seed の決定論を保つ）。
  insertRate?: number; // insert 帯の上限（既定 0.15）
  deleteRate?: number; // delete 帯の幅（既定 0.15＝[insertRate, insertRate+deleteRate)）
  sharedAnchorRate?: number; // seed 行を anchor に選ぶ確率（同一アンカー衝突を誘発・既定 0）
}
...
function genInsert(
  ...
  // DD-021-2: 一定確率で seed 由来の共有 anchor を選ぶ（複数 client が同一アンカーへ同時 Insert＝受付順収束の網羅）。
  // sharedAnchorRate===0（既定）のときは rng() を消費しない＝既存 seed の RNG 系列を変えない（determinism 非退行）。
  const liveShared = sharedAnchors.filter((a) => liveRows.includes(a));
  if (sharedAnchorRate > 0 && liveShared.length > 0 && rng() < sharedAnchorRate) {
    anchor = liveShared[Math.floor(rng() * liveShared.length)];
  } else if (liveRows.length > 0 && rng() >= 0.2) {
...
function generateOp(
  ...
  hot: HotCell[],
  sharedAnchors: RowId[] = [],
): GeneratedOp | undefined {
  const session = client.session;
  const view = session.viewDocument;
  const liveRows = displayRowOrder(view);
  const totalRows = view.rowOrder.length;
  const roll = rng();
  const insertBand = opts.insertRate ?? 0.15;
  const deleteBand = insertBand + (opts.deleteRate ?? 0.15);
  const anchorRate = opts.sharedAnchorRate ?? 0;

  if (liveRows.length < opts.minLiveRows && totalRows < opts.maxRows) {
    return { op: genInsert(client, liveRows, rng, sharedAnchors, anchorRate), occAttempt: false };
  }
  if (roll < insertBand && totalRows < opts.maxRows) {
    return { op: genInsert(client, liveRows, rng, sharedAnchors, anchorRate), occAttempt: false };
  }
  if (roll < deleteBand && liveRows.length > opts.minLiveRows) {
...
function runConvergence(opts: RunOptions): RunResult {
  ...
  // DD-021-2: 共有 anchor 候補（seed 行）。sharedAnchorRate>0 のとき複数 client がここへ同時 Insert し受付順収束を網羅する。
  const sharedAnchors: RowId[] = seededRowIds.map((r) => createRowId(r));
  ...
  for (let i = 0; i < opts.opCount; i += 1) {
    ...
    const generated = generateOp(actor, rng, opts, hot, sharedAnchors);
...
// ---------------------------------------------------------------------------
// DD-021-2: 行操作比率を強化した randomized 収束（同一アンカー衝突バイアス・insert/delete 高比率）。
// 既存 seed とは別 seed・別 describe で追加し、上の determinism ベースラインは無変更に保つ。
// scenarios.md S9: setCells/insert/delete ミックス・hash 一致・二重適用0・insert/delete 実適用≥1・seed 再現性。
// ---------------------------------------------------------------------------
const ROW_HEAVY_SEEDS: Array<{ seed: number; clientCount: number; opCount: number }> = [
  { seed: 21_020_201, clientCount: 3, opCount: 700 },
  { seed: 21_020_202, clientCount: 4, opCount: 700 },
  { seed: 21_020_203, clientCount: 5, opCount: 600 },
];

describe('invariant/collab（DD-021-2）: 行操作強化 randomized（同一アンカー衝突・insert/delete 高比率）', () => {
  for (const { seed, clientCount, opCount } of ROW_HEAVY_SEEDS) {
    it(
      `seed=${seed}・${clientCount}クライアント×${opCount}op（insert 0.30/delete 0.20・共有 anchor 0.5）→ 全 client 収束・二重適用0`,
      () => {
        const opts: RunOptions = {
          seed,
          clientCount,
          ...
          minLiveRows: 6,
          hotCells: 3,
          insertRate: 0.3, // insert 帯 [0, 0.30)
          deleteRate: 0.2, // delete 帯 [0.30, 0.50)
          sharedAnchorRate: 0.5, // 挿入の半分は seed 共有 anchor へ（同時同一アンカー Insert を頻発させる）
        };
        const run = runConvergence(opts);
        logConvergence(`row-heavy-${seed}`, run, seed, clientCount, opCount);
        try {
          assertInvariants(`row-heavy-${seed}`, run);
          // 行操作が実際に多く適用されたことを固定（「通るように薄めた」テストの否定）。
          expect(run.insertOps, `[row-heavy-${seed}] insert applied heavily`).toBeGreaterThanOrEqual(10);
          expect(run.deleteOps, `[row-heavy-${seed}] delete applied heavily`).toBeGreaterThanOrEqual(5);
        } catch (error) {
          process.stdout.write(
            `\n[COLLAB-INVARIANT ROW-HEAVY FAILED] reproduce with: seed=${seed} clients=${clientCount} ops=${opCount}\n`,
          );
          throw error;
        }
      },
  ...
  it('行操作強化 seed の再実行で最終 hash・operationLog trace が一致（seed 再現性）', () => {
    const opts: RunOptions = {
      seed: 21_020_299,
      clientCount: 4,
      opCount: 600,
      ...
      insertRate: 0.3,
      deleteRate: 0.2,
      sharedAnchorRate: 0.5,
    };
    const first = runConvergence(opts);
    const second = runConvergence({ ...opts });
    assertInvariants('row-heavy-det-1', first);
    assertInvariants('row-heavy-det-2', second);
    expect(second.operationLogDigest, 'row-heavy operation log trace matches').toBe(first.operationLogDigest);
    expect(second.serverHash).toBe(first.serverHash);
    expect(second.finalRevision).toBe(first.finalRevision);
  });
});
